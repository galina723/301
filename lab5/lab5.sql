...
so.order_number, so.salesman_number,
sod.product_number 
from clients c 
...
-- them.
select c.client_number, c.client_name, count(so.Order_Number) numOrder
from clients c inner join salesorder so
on  c.Client_Number = so.Client_Number 
...
from salesman sm 
inner join salesorder so
on sm.Salesman_Number = so.Salesman_Number
inner join salesorderdetails sod
on sod.Order_Number = so.Order_Number
...
-- received “successful
select c.client_name, c.client_number, count(so.order_status) as numberOrder
from clients c 
inner join salesorder so
on c.Client_Number = so.Client_Number
...
select sm.salesman_name
from salesman sm 
where Salesman_Number in (select so.Salesman_Number
from salesorder so 
inner join salesorderdetails sod
...
select sm.salesman_name
from salesman sm
where Salesman_Name in (select so.Salesman_Number 
from salesorder so
inner join salesorderdetails sod 
on so.Order_Number = sod.Order_Number
...
-- for each customer
select p.product_name, p.Sell_Price, sm.salesman_name, so.Delivery_Status, sod.Order_Quantity
from salesorder so
...
inner join salesorder so on so.Salesman_Number = sa.Salesman_Number
where so.Order_Status = 'Cancelled'
)
order by Salary asc;
